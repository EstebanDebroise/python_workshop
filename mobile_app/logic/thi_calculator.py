from __future__ import annotations

from logic.thi_result import ThiResult


class ThiCalculator:

    @staticmethod
    def compute(temp_c: float, humidity_pct: float) -> float:
        t_f = 1.8 * temp_c + 32
        return t_f - (0.55 - 0.0055 * humidity_pct) * (t_f - 58)

    @staticmethod
    def result(temp_c: float, humidity_pct: float) -> ThiResult:
        thi = ThiCalculator.compute(temp_c, humidity_pct)
        if thi < 68:
            return ThiResult(thi, "confort", "green", "ok")
        if thi < 72:
            return ThiResult(thi, "vigilance", "amber", "warn")
        if thi < 79:
            return ThiResult(thi, "alerte", "orange", "warn")
        if thi < 84:
            return ThiResult(thi, "danger", "red", "danger")
        return ThiResult(thi, "urgence", "darkred", "danger")
